...
import { toggleMarketplaceFavorite } from '@/server/marketplace/creator';
...
export async function POST(
  ...
  if (!parsed.success) {
    return NextResponse.json({ message: '市场条目不存在。' }, { status: 400 });
  }

  try {
    return NextResponse.json(
      await toggleMarketplaceFavorite(contextResult.context, parsed.data.itemId),
    );
  } catch (error) {
